...
namespace daiet {
    ...
    static __rte_always_inline void store(daiet_hdr* daiet, TensorUpdate* tensor_update) {
        ...
        if (tensor_update->type == INT) {
            ...
            for (uint32_t i = 0; i < daiet_par.getNumUpdates(); i++) {
                tensor_update->ptr.int_ptr[tsi] = int32_t(rte_be_to_cpu_32(entry->upd));
                entry++;
                tsi++;
            }
            ...
            for (uint32_t i = 0; i < daiet_par.getNumUpdates(); i++) {
                tensor_update->ptr.float_ptr[tsi] = (int32_t(rte_be_to_cpu_32(entry->upd))) / daiet_par.getScalingFactor();
                entry++;
                tsi++;
            }
        }
    ...
    static __rte_always_inline void reset_pkt(struct ether_hdr * eth, unsigned portid, uint32_t tsi, TensorUpdate* tensor_update, uint64_t ol_flags) {
        ...
        if (tensor_update->type == INT) {
            ...
            for (uint32_t i = 0; i < daiet_par.getNumUpdates(); i++) {
                entry->upd = rte_cpu_to_be_32(tensor_update->ptr.int_ptr[tsi]);
                entry++;
                tsi++;
            }
        } else if (tensor_update->type == FLOAT) {
            ...
            for (uint32_t i = 0; i < daiet_par.getNumUpdates(); i++) {
                entry->upd = rte_cpu_to_be_32(round((tensor_update->ptr.float_ptr[tsi]) * daiet_par.getScalingFactor()));
                entry++;
                tsi++;
            }
        }
    ...
    static __rte_always_inline void build_pkt(rte_mbuf* m, unsigned portid, uint32_t tsi, TensorUpdate* tensor_update) {
        ...
        while (!force_quit) {
            ...
            if (lcore_id == rte_get_master_lcore())
                ...
                for (j = 0; j < burst_size; j++) {
                    m = pkts_burst[j];
                    build_pkt(m, dpdk_par.portid, daiet_par.getNumUpdates() * j, tuptr);
                ...
                for (j = 0; j < nb_rx; j++) {
                    ...
                    if (likely(daiet!=NULL)) {
                        ...
                        if (likely(tsi < tensor_size)) {
                            ...
                            //Resend the packet
                            reset_pkt(eth, dpdk_par.portid, tsi, tuptr, m->ol_flags);
