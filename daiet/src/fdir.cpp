...
namespace daiet {
    ...
    void fdir_init(uint8_t port_id) {

        int ret;
        struct rte_eth_fdir_info fdir_info;

        // Check if FDIR filter is supported
        ret = rte_eth_dev_filter_supported(port_id, RTE_ETH_FILTER_FDIR);
        if (ret < 0)
            LOG_FATAL("FDIR not supported!");

        ret = rte_eth_dev_filter_ctrl(port_id, RTE_ETH_FILTER_FDIR, RTE_ETH_FILTER_NOP, NULL);

        // Get filter info
        rte_eth_dev_filter_ctrl(port_id, RTE_ETH_FILTER_FDIR, RTE_ETH_FILTER_INFO, &fdir_info);

        // Check if FDIR is activated
        if (fdir_info.mode == RTE_FDIR_MODE_PERFECT)
            LOG_INFO("MODE PERFECT RULE ENABLED");
        else if (fdir_info.mode == RTE_FDIR_MODE_SIGNATURE)
            LOG_INFO("MODE SIGNATURE ENABLED");
        else
            LOG_FATAL("FDIR not enabled!");

        if (ret < 0) {
            LOG_FATAL("FDIR FILTER not supported: " + string(strerror(rte_errno)));
        }
    }
